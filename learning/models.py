...
class AvatarVideo(models.Model):
    ...
    video_file = models.FileField(
        ...
        null=True,
        blank=True,
        help_text='Permanent video file storage'
    )
    ...
    def is_expired(self):
        """
        Check if the D-ID temporary URL has expired
        D-ID URLs typically expire after 3-7 days
        """
        from django.utils import timezone
        from datetime import timedelta
        
        # If we have a permanent file, never expired
        if self.video_file:
            return False
